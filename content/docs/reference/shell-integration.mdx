...
description: What Terax injects into your shell, the OSC sequences it watches, and how to opt out.
icon: CommandLine
---
...
A Terax-specific extension. Emitted by terminal agents (Claude Code hooks) to signal state transitions: `started`, `working`, `attention`, `finished`, `exited`. The PTY byte filter (`pty/agent_detect.rs`) self-arms on `OSC 133;C` and reads `OSC 777` payloads to drive the [notification router](/docs/features/notifications).
